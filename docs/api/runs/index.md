...
3. Call [Trigger run](trigger-run.md) with `graph_id` and optional `inputs`.
4. Poll [Get run](get-run.md) until `status` is terminal (`succeeded` or `failed`) or `counts` show all items finished.
