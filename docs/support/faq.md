...
??? question "Where can I try the API?"
    Use your organization's hosted [API Playground](../api/playground.md). The URL is client-specific (open it from Agate or use the host provided for your organization). It loads your tenant's public OpenAPI contract and keeps an entered key in tab-scoped session storage, where it survives reloads until you clear it, sign out, or close the tab.
